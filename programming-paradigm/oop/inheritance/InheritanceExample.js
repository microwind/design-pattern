...
class Manager extends Employee {
    ...
    // 覆写父类的方法，打印经理信息
    displayInfo() {
        console.log(`Manager: ${this.name}, 薪水: ${this.salary}, 部门: ${this.department}`);
    }
...
class Engineer extends Employee {
    constructor(name, salary, expertise) {
        super(name, salary); // 调用父类的构造方法
    ...
    // 覆写父类的方法，打印工程师信息
    displayInfo() {
        console.log(`Engineer: ${this.name}, 薪水: ${this.salary}, 职位: ${this.expertise}`);
    }
...
// 创建基类对象：雇员
let emp = new Employee("Tom", 50000);
emp.displayInfo(); // 输出：Employee: Tom, 薪水: 50000

// 创建子类对象：经理
let mgr = new Manager("Jack", 80000, "Engineering");
mgr.displayInfo(); // 输出：Manager: Jack, 薪水: 80000, 部门: Engineering
mgr.manageTeam(); // 输出：Jack 管理的部门.

// 创建子类对象：工程师
let eng = new Engineer("Charlie", 60000, "Software Development");

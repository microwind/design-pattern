...
面向对象程序设计有三要素：封装、继承、多态，前两者较好理解，多态总让人困惑，不知道具体有什么作用，为什么要有多态。今天来详细分析下什么是多态，以及多态有哪些好处。
...
## 多态的好处
在面向对象设计中，“开闭原则”是非常重要的一条。即系统中的类应该对扩展开放，对修改关闭。这样设计的代码更易于扩展和维护。

继续延续上面的例子，假设业务需要扩充更多子类，我们可以通过以下步骤来体现开闭原则：

1. 新增子类：根据业务需求，新增符合现有类层次结构的子类，例如 AnotherChild。

2. 继承和重写：新的子类应该继承自适当的父类，并根据需要重写父类的方法或添加新方法。

3. 不修改现有代码：遵循开闭原则，我们不修改现有的 Parent 和 Child 类的代码。

4. 使用多态：依然使用父类引用来声明子类对象，例如 Parent child = new AnotherChild();，这样代码中的现有逻辑不需要改变。

5. 编译时不变性：编译时确定方法调用的特性不改变，仍然根据引用类型来确定方法的名称和参数。

6. 运行时多态性：运行时根据实际对象的类型来决定执行的方法，这符合面向对象设计中的多态性原则，使得代码具有良好的可扩展性和可维护性。

通过这种方式，我们在不修改现有代码的情况下，可以轻松地引入新的子类并扩展系统功能，同时保持现有代码的稳定性和可靠性，符合开闭原则的设计理念。
```java
// 定义一个通用Animal类
class Animal {
    void makeSound() {

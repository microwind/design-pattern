...
// TS是JS的语法糖，最后编译成JS运行。其模仿Java的语法特性，实现了类型的概念。
// 在多态上，TS跟Java较为接近，但又有不同。
...
// 创建动物数组，无需转型，都是成员
const animals: Animal[] = [dog, cat, pig, new Animal()]
...
animals.forEach((animal) => {
  ...
  if (animal instanceof Dog) {
    ;(animal as Dog).bark() // 调用Dog类的bark方法
  } else if (animal instanceof Cat) {
    ;(animal as Cat).meow() // 调用Cat类的meow方法
  } else if (animal instanceof Pig) {
    ;(animal as Pig).oink() // 调用Pig类的oink方法
  }
})

/*** 分割线 ***/

const dogPtr: Animal = new Dog() // 向上转型
const catPtr: Animal = new Cat() // 向上转型
const pigPtr: Animal = new Pig() // 向上转型

// 多态特性，在编译时按引用类型确定，在运行时根据对象的实际类型来确定，而不是引用的类型。
dogPtr.eat() // 调用Dog类的eat方法
;(dogPtr as Dog).bark() // 类型强制转换并调用Dog类的bark自有方法

catPtr.eat() // 调用Cat类的eat方法
;(catPtr as Cat).meow() // 类型强制转换并调用Cat类的meow自有方法

pigPtr.eat() // 调用Pig类的eat方法
;(pigPtr as Pig).oink() // 类型强制转换并调用Pig类的oink自有方法

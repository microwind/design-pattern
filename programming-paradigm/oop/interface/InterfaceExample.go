...
type AbstractAlarm struct{}

func (a AbstractAlarm) trigger() {
  // 空方法体，具体报警器实现时需覆盖该方法
}
...
func (sd SecurityDoor) open() {
  fmt.Println("安全门打开")
}

func (sd SecurityDoor) close() {
  fmt.Println("安全门关闭")
}

// 覆盖父类的trigger方法，实现具体的报警逻辑
func (sd SecurityDoor) trigger() {
  fmt.Println("警报触发! 有一条安全告警。")
}

func main() {
  // 创建安全门对象
  var securityDoor SecurityDoor
...
/*
jarry@jarrys-MacBook-Pro interface % go run InterfaceExample.go
安全门打开
安全门关闭
警报触发! 有一条安全告警。
*/

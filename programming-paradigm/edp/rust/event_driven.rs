...
// 事件管理器结构体
struct EventManager {
    events: HashMap<u32, Vec<Callback>>,
}
...
impl EventManager {
    ...
    // 注册事件监听器
    fn on(&mut self, event_name: u32, callback: Callback) {
        self.events
            .entry(event_name)
            .or_insert(Vec::new())
            .push(callback);
    ...
    // 触发事件
    fn emit(&self, event_name: u32, message: &str) {
    ...
    // 移除事件监听器
    fn off(&mut self, event_name: u32, callback: Callback) {
...
fn main() {
    ...
    println!("第一次测试：");
    // 注册事件监听器
    event_manager.on(0, handle_message);
    // 触发事件
    event_manager.emit(0, "第一次发送的消息");

    // 第二次测试：再次触发事件
    println!("\n第二次测试：");
    event_manager.emit(0, "第二次发送的消息");

    // 第三次测试：移除监听器后重新注册并触发事件
    println!("\n第三次测试：");
    // 移除事件监听器
    event_manager.off(0, handle_message);
    // 重新注册事件监听器
    event_manager.on(0, handle_message);
    // 触发事件
    event_manager.emit(0, "第三次发送的消息");
}
